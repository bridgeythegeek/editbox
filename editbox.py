...
class EditBox(messagehooks.MessageHooks):
    ...
    #-------------------------------------------------------------------
    def dump_edit(self, dump_to_file, atom_class_name, context, outfd, wnd, task):
        """Process an Edit"""
        ...
        isPassword = (editbox.pwdChar != 0x00)
        ...
        if self._config.minimal:
            ...
            outfd.write("address-of cbwndExtra: {0:#x} [{1:#x}]\n".format(wnd.cbwndExtra.obj_offset, task_space.vtop(wnd.cbwndExtra.obj_offset)))
            ...
            outfd.write("pointer-to hBuf      : {0:#x} [{1:#x}]\n".format(editbox.get_hBuf(), task_space.vtop(editbox.get_hBuf())))
    ...
    def dump_listbox(self, dump_to_file, atom_class_name, context, outfd, wnd, task):
        ...
        outfd.write("stringsStart         : {0:#x} [{1:#x}]\n".format(listbox.stringsStart, task_space.vtop(listbox.stringsStart)))
        ...
        for winsta, atom_tables in data:
            for desktop in winsta.desktops():
                for wnd, _level in desktop.windows(desktop.DeskInfo.spwnd):
                    ...
                    if self._config.pid == None or int(wnd.Process.UniqueProcessId) in the_tasks:
